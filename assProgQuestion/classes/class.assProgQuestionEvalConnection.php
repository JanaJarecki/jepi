 ...
 */
class assProgQuestionEvalConnection {
	...
	static function compileTestNG($question, $code = NULL) {
		...
		$evaluator = new assProgQuestionEvalConnection ($question, $code, $action);
	...
	static function runTestNG($question, $code = NULL) {
		...
		$evaluator = new assProgQuestionEvalConnection ($question, $code, "runtestng", array());
		$evaluator->createTestNGXML();
	...
	static function runStudentTestNG($question, $code, $params) {
		$evaluator = new assProgQuestionEvalConnection ($question, $code, "runstudenttestng", $params);
		$evaluator->createTestNGXML();
	...
	static function feedbackStudentTestNG($question, $code, $params) {
		$evaluator = new assProgQuestionEvalConnection ($question, $code, "feedbackstudenttestng", $params);
		$evaluator->createTestNGXML();
	...
	static function runCode($question, $code = NULL, $owner, $params = array()) {
		$action = 'run' . $owner;
		$evaluator = new assProgQuestionEvalConnection ($question, $code, $action, $params);
	...
	function __construct($question, $code = NULL, $action = 'run', $params = array()) {
		...
		$this->check_method_type = $this->question->getCheckRecursive() || $this->question->getCheckIterative()
			|| $this->question->getForbidRecursive()
			|| $this->question->getForbidIterative();
	}
